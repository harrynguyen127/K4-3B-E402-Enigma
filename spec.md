...
- Mức prototype nhắm tới: [ ] Sketch [ ] Mock [x] Working
  ...
  - **Cập nhật CP3 lần 2 (18/09, theo giao diện VLearn thật):** "Kiểm tra" = nộp câu, đáp án khoá, không chọn lại — sửa sai qua câu làm lại do AI sinh. Gợi ý trước-khi-nộp mặc định ẩn, lấy từ `codebase/js/data.hints.js` do AI sinh sẵn một lần (20 câu × 3 persona, `server/scripts/generate-hints.js`, prompt không chứa đáp án) để không tốn lời gọi lúc học. Sidebar có "Tiến độ lượt này" và "Kiến thức đang luyện" (`concept`, bản nháp cần review). Mỗi khối AI có nút 👍/👎 + lý do → `server/logs/feedback.jsonl` gắn `trace_id`.
