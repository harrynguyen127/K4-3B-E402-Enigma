...
- Golden set (`eval/golden_set.json`, 24 case nhóm tự xây; cơ cấu và expected nằm trong `eval/README.md`/`user-input-grid.md`):
...
| Lần chạy | Số case | Điểm trung bình so với gold set | Phương pháp | Ghi chú |
|---|---:|---:|---|---|
| Baseline | 24 | **55%** | LLM judge so sánh output với đáp án chuẩn | Điểm sơ bộ, chưa có human validation |
| Current | 24 | **87%** | LLM judge so sánh output với đáp án chuẩn | Sau khi cải thiện prompt/model/pipeline; chưa có human validation |

- Phần chưa hoàn tất:
  - Chưa có vòng chấm thủ công độc lập của các thành viên nhóm; 55% và 87% chưa phải điểm human-validated.
  - Chưa lưu bảng report đầy đủ theo từng case/từng chiều vào `eval/run-01-results.md`; cần ghi cả case đạt, case trượt và nguyên nhân.
  - Chưa có khảo sát/PV chuẩn A với học viên ngoài nhóm; các con số impact hiện dùng mining proxy và phải ghi rõ khi trình bày.
